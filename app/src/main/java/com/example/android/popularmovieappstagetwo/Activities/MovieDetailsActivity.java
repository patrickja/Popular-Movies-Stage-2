package com.example.android.popularmovieappstagetwo.Activities;
...
import com.example.android.popularmovieappstagetwo.Adapters.TrailerAdapter;
...
import com.example.android.popularmovieappstagetwo.R;
import com.example.android.popularmovieappstagetwo.Utilities.AppExecutors;
import com.example.android.popularmovieappstagetwo.Utilities.JsonUtils;
import com.example.android.popularmovieappstagetwo.Utilities.NetworkUtils;
...
public class MovieDetailsActivity extends AppCompatActivity implements TrailerAdapter.ListItemClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (movieItem == null) {
            closeOnError(getString(R.string.movie_error));
            return;
        }
        ...
        mFavButton = findViewById(R.id.fab);
        mDb = MovieDatabase.getInstance(getApplicationContext());

        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                final Favorites fmov = mDb.movieDao().loadMovieById(Integer.parseInt(movieItem.getId()));
    ...
    private void setFavorite(Boolean fav) {
        if (fav) {
            isFav = true;
            mFavButton.setImageResource(R.drawable.ic_like_post);
        } else {
            isFav = false;
            mFavButton.setImageResource(R.drawable.ic_like_pre);
        }
    }
    ...
    private void getMoreDetails(String id) {
        ...
        SearchURLs searchURLs = new SearchURLs(
                NetworkUtils.buildUrl(reviewQuery, Constants.apiKey),
                NetworkUtils.buildUrl(trailerQuery, Constants.apiKey)
        );
    ...
    private void populateDetails() {
        ...
        ((TextView) findViewById(R.id.tv_movie_title)).setText(movieItem.getTitle());
        ((TextView) findViewById(R.id.tv_movie_vote_average)).append(" (" + movieItem.getVote() + "/10)");
        ((RatingBar) findViewById(R.id.rb_movie_vote_average)).setRating(Float.parseFloat(movieItem.getVote()));
        ((TextView) findViewById(R.id.tv_release_date)).setText(movieItem.getReleaseDate());
        ((TextView) findViewById(R.id.tv_movie_overview)).setText(movieItem.getSynopsis());

        mFavButton.setOnClickListener(new View.OnClickListener() {
            //@Override
            public void onClick(View v) {
                final Favorites mov = new Favorites(
        ...
        try {
            Picasso.get()
                    ...
                    .into((ImageView) this.findViewById(R.id.img_thumbnail));
        } catch (Exception ex) {
